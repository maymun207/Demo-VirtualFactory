# STEP 4 — Supabase Migration (RPC Functions)

> **⚠️ IMPORTANT: Before running this step with AntiGravity, YOU must manually run the SQL from Section 4.2 in your Supabase SQL Editor. Without this, the CWF agent cannot query the database.**

---

## RULES

1. This step creates ONE file: the migration SQL for version control.
2. **YOU must run the SQL manually** in Supabase SQL Editor.
...
|--------|-----|-------------|
| Run SQL in Supabase | **YOU (manual)** | Creates `execute_readonly_query` and `get_simulation_stats` RPC functions |
| Create migration file | AntiGravity | Saves the SQL to `supabase/migrations/` for version control |

---

## 4.1 Manual Step: Run in Supabase SQL Editor

1. Open your Supabase Dashboard: https://supabase.com/dashboard
2. Select your project (`ukhattgmidhchanzvevt`)
3. Go to **SQL Editor**
4. Paste the ENTIRE SQL from Section 4.2 below
5. Click **Run**
6. Verify with: `SELECT execute_readonly_query('SELECT 1 as test');`
   - Expected result: `[{"test": 1}]`
7. Verify with: `SELECT get_simulation_stats('any-uuid-from-your-simulations');`
   - Expected: JSON object with session, tile_counts, etc.

---

## 4.2 The SQL Migration
...
## 4.3 Create Migration File (for AntiGravity)

**File to create:** `supabase/migrations/20260225_cwf_agent_rpc.sql`

The file content is exactly the SQL from Section 4.2 above. This is for version control only — the actual execution happens manually in Supabase SQL Editor.
...
- [ ] `SELECT execute_readonly_query('SELECT 1 as test');` → returns `[{"test": 1}]`
- [ ] `SELECT execute_readonly_query('DELETE FROM tiles');` → returns ERROR (forbidden keyword)
- [ ] `SELECT get_simulation_stats('some-uuid');` → returns JSON object
- [ ] Migration file saved at `supabase/migrations/20260225_cwf_agent_rpc.sql`

---

**NEXT:** Proceed to STEP-5 for deployment and testing.
