 ...
 * across all 7 production stations. When a parameter's live value (from
 * currentParams in simulationDataStore) falls OUTSIDE its range, the
 * defect engine (defectEngine.ts) can inject a defect on the tile
 ...
 *   - Used by: defectEngine.ts (evaluateStationDefects)
 ...
 */
export const PARAMETER_RANGES: Record<StationName, Record<string, ParameterRange>> = {
